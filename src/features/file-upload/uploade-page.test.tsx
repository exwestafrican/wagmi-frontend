...
describe("UploadPage", () => {
	...
	test("user can upload one file", async () => {
		...
		fireEvent.change(fileInput, {
			target: { files: createFileListMock(mockFile) },
		})
	...
	test("uploading an extra file overwrites previous upload", async () => {
		...
		fireEvent.change(fileInput, {
			target: { files: createFileListMock(firstMockFile) },
		})
		...
		fireEvent.change(fileInput, {
			target: { files: createFileListMock(secondMockFile) },
		})
	...
	test("user cannot upload file exceeding size limit", async () => {
		...
		// Trigger file upload
		fireEvent.change(fileInput, {
			target: { files: createFileListMock(largeMockFile) },
		})
